...
function draw(){
  $("#list").innerHTML=filtered.map(p=>`
    <li>
      <button type="button" data-d="${p.d}" aria-label="Open ${p.e} name research">
...
function show(d){
  ...
  $("#pos").textContent=`Entry ${p.d} of ${DATA.length}`;
  ...
  $("#browse").hidden=true;
  $("#detail").hidden=false;
  document.title=`${p.e} name origins · Poké Etymology`;
  scrollTo(0,0);
}
...
function closeEntry(){
  ...
  $("#detail").hidden=true;
  $("#browse").hidden=false;
  document.title="Poké Etymology · Pokémon names across languages";
